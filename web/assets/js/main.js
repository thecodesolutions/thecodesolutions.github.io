(function($) {
    ...
    function awdCanvasInit() {
        for (var i = 0; i < awd_bg_number_of_curves; i++) {
            ...
            curves_array.push(new curve(abc1x, abc1y, abc2x, abc2y, x, y, abc1xvx, abc1xvy, abc1yvx, abc1yvy, abc2xvx, abc2xvy, abc2yvx, abc2yvy));
        }
    }
    ...
    function awdCanvasDraw() {
        ...
        for (var i = 0; i < curves_array.length; i++) {
            ...
            ctx.bezierCurveTo(curves_array[i].abc1x, curves_array[i].abc1y, curves_array[i].abc2x, curves_array[i].abc2y, canvas.width + 100, curves_array[i].y - 100);
            ctx.stroke();
    ...
    function awdContentSlider() {
        ...
        var menu_link = menu.find('a');
		 $('#awd-site-logo').click(function(){            
            var headersClass = $('#header-overlay').attr('class');
            $("#header-overlay").removeClass(headersClass);
        });
        menu_link.on('click', function(e) {
            ...
            var active_bg = bg.find('.bg-' + active_slide);
             ...
             var headersClass = $('#header-overlay').attr('class');
             $("#header-overlay").removeClass(headersClass);
             $("#header-overlay").addClass('bg-' + active_slide);
             awdAnimate('bg-' + active_slide);
             //header backgroung --------->     
    ...
    function awdCountdown() {
        $('#clock').countdown(awd_countdown_date).on('update.countdown', function(event) {
            var $this = $(this).html(event.strftime('' + '<div class="counter-container"><div class="counter-date"><div class="counter-box first"><span>day%!d</span><div class="number">%-D</div></div></div>' + '<div class="counter-time"><div class="counter-box"><div class="number">%H:%M</div></div>' + '<div class="counter-box last"><div class="number">%S</div><span>seconds</span></div></div></div>'));
        });
    }
